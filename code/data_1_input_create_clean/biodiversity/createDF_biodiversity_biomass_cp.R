...
##### input files #####
f_biomass <- '/eos/jeodpp/data/projects/FOREST-RESILIENCE/data/biodiversity/biomass/nasa_biomass/agbRescaledAtModis.tif'
f_forcov  <- '/eos/jeodpp/data/projects/FOREST-RESILIENCE/data/ancillary/hansen/hansen_forest_cover_nc_mask/hansenForestCoverNoLoss2000AtModisMean_binary50.nc'

# initialise output
output_path <- '/eos/jeodpp/data/projects/FOREST-RESILIENCE/data/biodiversity/biomass/nasa_biomass/'
...
r_glcm_dissim_count ; plot(r_glcm_dissim_count) ; summary(r_glcm_dissim_count)  ; 
...
# aggregate to 5km - then need to rematch to get it to match the df of rest of data
...
print(cor(df_biomass_diversity$glcm_dissimilarity, df_biomass_diversity$glcm_homogeneity))
...
head(df_comb)
df_comb_biomass <- left_join(df_comb, df_biomass_diversity)
summary(df_comb_biomass)
df_forcov <- as.data.frame(r_forcov, xy=T, na.rm=T)
df_comb_biomass <- left_join(df_biomass_diversity, df_forcov)
