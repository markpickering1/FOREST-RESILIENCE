...
######     GLOBAL VARS                        #####
script_info <- 'forgenius_kndvi_extract_points'               # used in output name
...
t_i_baseVar  <- terra::rast(input_kndvi) #, varname = 'kndvi') ; plot(r_i_baseVar[[1]])
...
# loop over the 
for() i in length() ) 
t_i_baseVar_1 <- t_i_baseVar[[1]]
head(v_gcu)
vect_gcu <- vect(v_gcu)
crs(v_gcu)  <-  crs(t_i_baseVar)
...
# v_gcu_rast_1 <- as.data.frame(v_gcu_rast_1)

# next divide by 10000

# save 
save(v_gcu_rast_1, file=paste0(output_path, 'df_kndvi_ts_gcuPoints.RData' )    )
