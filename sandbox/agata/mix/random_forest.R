...
#load("/eos/jeodpp/data/projects/FOREST-RESILIENCE/GIT-FOREST-RESILIENCE/FOREST-RESILIENCE/sandbox/agata/df_all_inner_muTACcov.RData")
#variables <- df_kndvi

# Remove na? Not necessary if they were already remove with inner join
#variables <- na.omit(variables)
  
# Explore the loaded Rdata object
...
mtry_1 <- ceiling(length(v_predictors)/3) # use default and optimise
...
save(rf.model, file=paste0(output_path, 'rf_model_gioCp_spei.RData' )    )

# Define if going for a subset of the data to train rf model or for the whole data (here going for half of data)
train <- sample(1:nrow(variables), nrow(variables) /2)

# Define parameters of the random forest model
ntree <- 50
#mtry <-sqrt(ncol(variables))# mtry indicates how many predictors should be considered for each split of the tree
mtry <- ncol(variables)/3    # by default, randomForest() uses p/3 variables when building a random forest of regression 
                             # trees, and √p variables when building a random forest of classification trees (Introduction to statistical modelling book)

# Set seed when performing calculations involving random quantities to reproduce results
set.seed(1)
...
rf_model <- randomForest(tac_resid_kndvi ~ . - n_var_kndvi - mu_var_kndvi - sd_var_kndvi - cv_var_kndvi - cv_calc_var_kndvi, data = variables, ntree = ntree, mtry = mtry, importance = TRUE)
